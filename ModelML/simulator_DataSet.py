...
class VehicleDeamon(traci.StepListener):
    ...
    def reset_vehicle(self):
        try:
            ...
            #Actualizar valores de wear
            update_wear(self.veh_id)
        except traci.exceptions.TraCIException as e:
...
dic_wear = {veh: {"speed": 0.0, "fuel_consumption": 1.0, "engine_temperature": 0.0,  
            "time_steps": 0,  "phase": 0} for veh in VEH_IDS}
...
def update_wear(vehicle_id):
    ...
    if w["phase"] == 0:
        ...
        #if w["time_steps"]>= WEAR_DURATION + 2:
            #w["phase"] = 0
            #w["time_steps"] = 0
            #w["fuel_consumption"] = 1.0
            #w["engine_temperature"] = 0.0
            #w["speed"] = 0
...
# Añadir estructura en memoria para almacenar los datos por vehículo
dataset = []

def add_data(data, veh_id, max_len=20000):
    """Agregar 'data' como nueva fila en la lista dataset."""
    if not isinstance(data, dict):
        return
    if dic_wear[veh_id].get("phase") == 1:
        data["Maintenance"] = 1
    else:
        data["Maintenance"] = 0
...
def dump_dataset(path="dataset_wear.csv"):
    """Guardar todas las filas acumuladas en dataset como CSV."""
    import pandas as pd
    if not dataset:
        print("dataset vacío: no se creó CSV.")
        return
